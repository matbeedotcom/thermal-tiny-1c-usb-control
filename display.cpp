...
#ifdef OPENCV_ENABLE
    cv::Mat raw_image(height, width, CV_16UC1, stream_frame_info->image_frame);
    cv::Mat normalized_image;
    cv::normalize(raw_image, normalized_image, 0, 255, cv::NORM_MINMAX, CV_8UC1);
    cv::Mat color_image;
    cv::applyColorMap(normalized_image, color_image, cv::COLORMAP_JET);
	putText(color_image, frameText, cv::Point(11, 11), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar::all(0), 1, 8);
	putText(color_image, frameText, cv::Point(10, 10), cv::FONT_HERSHEY_PLAIN, 1, cv::Scalar::all(255), 1, 8);
